...
namespace AccessToExcel
{
    public partial class Frm_Main : Form
    {
        ...
        private void btn_Read_Click(object sender, EventArgs e)
        {
            try
            {
                string P_str_Con = @"Provider=Microsoft.Ace.OleDB.12.0;Data Source=" + txt_Access.Text + ";Persist Security Info=True";//记录连接Access的语句
                string P_str_Sql = "";//存储要执行的SQL语句
                OleDbConnection oledbcon = new OleDbConnection(P_str_Con);//实例化OLEDB连接对象
                OleDbCommand oledbcom;//定义OleDbCommand对象
                oledbcon.Open();//打开数据库连接
                //向Excel工作表中导入数据
                P_str_Sql = @"select * into [Excel 12.0;database=" + txt_Excel.Text + "]." + "[" + cbox_Table.Text + "] from " + cbox_Table.Text + "";//记录连接Excel的语句
            ...
            catch(Exception e1)
            {
               // Console.WriteLine(e1.Message);
                MessageBox.Show(e1.Message + "工作表已经存在，请选择其他数据表！", "警告", MessageBoxButtons.OK, MessageBoxIcon.Warning);
            }
        }
